<script lang="ts">
  ...
  interface Automation {
    ...
    config: Record<string, number | string>; lastRunOn: string | null;
  }
          ...
          <div class="mt-4 space-y-3 border-t border-slate-100 pt-4 dark:border-slate-800">
            ...
            <div class="flex flex-wrap items-center gap-3 pt-1">
              ...
              {:else}
                ...
                {#if it.mode === 'auto'}<span class="text-xs text-slate-400">{tr({ en: 'Also runs automatically each day.', ar: 'يعمل أيضاً تلقائياً كل يوم.' }, $locale)}</span>{/if}
              {/if}
