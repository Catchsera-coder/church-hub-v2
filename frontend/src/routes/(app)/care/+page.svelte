<script lang="ts">
  ...
  function startNew() { editing = { type: 'prayer', subject: '', details: '', status: 'open', confidential: false, personId: null, personName: '', assignedToUserId: null, dueOn: '' }; error = ''; }
  function startEdit(r: any) { editing = { ...r, personName: r.personGiven ? displayName({ givenName: r.personGiven, familyName: r.personFamily }, $nameOrder, $locale) : '', dueOn: r.dueOn ?? '' }; error = ''; }
  ...
  async function save() {
    ...
    try {
      const body = {
        ...
        status: editing.status, confidential: !!editing.confidential,
...
<PageHint id="care" text={{ en: 'Track prayer requests, care needs, visits and follow-up tasks. Assign each to a pastor, set a due date, and mark confidential items so their details stay private. Move an item Open → In progress → Done as you care for people.', ar: 'تابع طلبات الصلاة واحتياجات الرعاية والزيارات والمهام. عيّن كلاً لراعٍ، وحدّد تاريخ استحقاق، وميّز السرّي ليبقى خاصاً. انقل العنصر من مفتوح إلى قيد التنفيذ إلى منجز.' }} />
...
{#if editing}
  <div class="card mb-4 space-y-3 p-5">
    ...
    <div class="grid gap-3 sm:grid-cols-2">
      ...
      <label class="flex items-center gap-2 pt-6 text-sm"><input type="checkbox" bind:checked={editing.confidential} /> 🔒 {tr({ en: 'Confidential (hide details from other staff)', ar: 'سرّي (إخفاء التفاصيل عن باقي الفريق)' }, $locale)}</label>
    </div>
        ...
        <div class="flex flex-wrap items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              ...
              {#if r.confidential}<span title={tr({ en: 'Confidential', ar: 'سرّي' }, $locale)}>🔒</span>{/if}
            ...
            {#if r._redacted}<p class="mt-1 text-xs italic text-slate-400">🔒 {tr({ en: 'Confidential — details hidden.', ar: 'سرّي — التفاصيل مخفية.' }, $locale)}</p>{:else if r.details}<p class="mt-1 whitespace-pre-wrap text-sm text-slate-600 dark:text-slate-300">{r.details}</p>{/if}
