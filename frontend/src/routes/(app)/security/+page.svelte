<script lang="ts">
  ...
  async function enable() {
    ...
    try {
      ...
      recoveryCodes = r.data.recoveryCodes;
      view = 'codes';
    } catch (err) { error = err instanceof ApiError ? err.message : (err as Error).message; }
    finally { busy = false; }
  }
...
<PageHint id="security" text={{ en: 'Add two-factor authentication (2FA) to your account — a second step at sign-in so a stolen password alone can’t get in. Use any authenticator app (Google Authenticator, Microsoft Authenticator, Authy…), and keep your recovery codes somewhere safe.', ar: 'أضِف المصادقة الثنائية (2FA) لحسابك — خطوة ثانية عند تسجيل الدخول حتى لا تكفي كلمة المرور وحدها. استخدم أي تطبيق مصادقة واحتفظ برموز الاسترداد في مكان آمن.' }} />

<div class="mx-auto max-w-xl">
  {#if error}<p class="mb-4 rounded-md bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-900/30 dark:text-rose-300">{error}</p>{/if}
  ...
  {:else if view === 'setup'}
    <div class="card space-y-4 p-6">
      ...
      <ol class="list-inside list-decimal space-y-1 text-sm text-slate-600 dark:text-slate-300">
        <li>{tr({ en: 'Open your authenticator app and add a new account.', ar: 'افتح تطبيق المصادقة وأضف حساباً جديداً.' }, $locale)}</li>
