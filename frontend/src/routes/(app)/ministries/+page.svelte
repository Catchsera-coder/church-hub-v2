...
<PageHeader title={$t('nav.ministries')}>
  {#snippet actions()}
    {#if can('create ministry')}<a href="/ministries/new" class="btn-primary">{$t('common.new')}</a>{/if}
  {/snippet}
</PageHeader>
...
<DataTable {loading} {rows} headers={[tr({ en: 'Ministry', ar: 'الخدمة' }, $locale), tr({ en: 'Age group', ar: 'الفئة' }, $locale), tr({ en: 'Schedule', ar: 'الجدول' }, $locale)]}>
  {#snippet row(m)}
    <td class="p-3 font-medium">
      {#if editable}<a class="text-primary-700 hover:underline dark:text-primary-300" href="/ministries/{m.id}">{tr(m.name, $locale)}</a>{:else}{tr(m.name, $locale)}{/if}
    </td>
